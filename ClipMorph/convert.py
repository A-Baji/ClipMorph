...
def convert_to_short_form(
    ...
    with mpy.VideoFileClip(input_path) as clip:
        ...
        if include_cam:
            cam_feed = Crop(
                x1=cam_x, y1=cam_y, width=cam_width, height=cam_height
            ).apply(clip)
        ...
        if not include_cam:
            ...
            bg_blur = Crop(
                ...
                y_center=bg_full.h // 2
            ).apply(bg_blur)

            bg_h = cam_h // 2
            bg_blur_top = Crop(x1=0, y1=0, width=crop_width, height=bg_h).apply(bg_blur)
            bg_blur_bot = Crop(x1=0, y1=bg_h + main_clip.h, width=crop_width, height=bg_h).apply(bg_blur)
