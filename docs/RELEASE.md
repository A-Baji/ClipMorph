...
- `.github/workflows/release.yml` is the single release pipeline. Run it manually with a version `X.Y.Z`; it updates `main`, builds and smoke-tests two executable variants (CLI and UI) for Windows, macOS, and Linux from one commit, then creates the annotated tag `vX.Y.Z` and GitHub Release only after the builds succeed. Python users install from the tagged GitHub repository URL.
